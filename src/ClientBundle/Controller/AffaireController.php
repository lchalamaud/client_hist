...
{
    /**
     *@Route("/affaire/table/", name="affaire_tab")
     */
    public function Affaire(Request $request)
    {
       ...
       if ($request->isMethod('POST')) {
        ...
        $timeStep = explode(":",$config[0])[1];
        $commercialPref = explode(":",$config[1])[1];
        $etatPref = explode(":",$config[2])[1];
        ...
        return $this->render('@Client/Default/affaireTable.html.twig', $response);
    }
    ...
    public function supprAffaire($id)
    {
        ...
        $rptTache = $em->getRepository('ClientBundle:Tache');
        $listTaches = $rptTache->findBy(['affaire' => $id]);
        foreach ($listTaches as $tache) {
            $em->remove($tache);
        }

        $rptAffaire = $em->getRepository('ClientBundle:Affaire');
    ...
    public function modifAffaire(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $rptAffaire = $em->getRepository('ClientBundle:Affaire');
        $rptComm = $em->getRepository('ClientBundle:Commercial');
